...
func ExampleSQLiteMenu_Get() {
	...
	err = sqlMenu.Connect(constants.KSQLiteConnectionString)
	if err != nil {
	...
	menu, length, err = sqlMenu.Get()
	if err != nil {
		log.Fatalf("sqlMenu.GetMainMenu().error: %v", err.Error())
	}

	if length != 4 {
		log.Fatalf("sqlMenu.GetMainMenu()..length.error: %v. Must be 4", length)
	}
